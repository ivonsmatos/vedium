...

{
  ...
  const levelHrefs = await page.$$eval('a[href^="/curso/portugues-para-estrangeiros-"]', (els) => [
    ...new Set(els.map((el) => el.getAttribute("href"))),
  ]);

  results.desktop = { faqExpandsOnClick: expandedAfterClick === "true", anchorScrollsToLevels: inViewport, levelHrefsFound: levelHrefs, errors };
  await page.close();
}
