...

{
  ...
  const levelHrefs = await page.$$eval('a[href^="/curso/espanhol-"]', (els) => [...new Set(els.map((el) => el.getAttribute("href")))]);
  const secondaryCount = await page.locator(".v2-insights__secondary").count();

  results.desktop = {
    faqExpandsOnClick: expandedAfterClick === "true",
    anchorScrollsToLevels: inViewport,
    levelHrefsFound: levelHrefs,
    insightsSecondaryCount: secondaryCount,
    errors,
  };
  await page.close();
}
