// QA visual: screenshots full-page e por seção de /curso-de-ingles-online
...
const GROUPS_1440 = [
  ...
  { name: "02-ponto-de-partida", startIndex: 3, endIndex: 3 },
  { name: "03-live", startIndex: 4, endIndex: 4 },
  { name: "04-niveis", startIndex: 5, endIndex: 5 },
  { name: "05-objetivos", startIndex: 6, endIndex: 6 },
  { name: "06-progressao", startIndex: 7, endIndex: 7 },
...
for (const viewport of VIEWPORTS) {
  ...
  await page.screenshot({ path: `${OUT_DIR}/english-${viewport.name}-full.png`, fullPage: true });
  ...
  for (const group of viewport.groups) {
    ...
    await page.screenshot({
      path: `${OUT_DIR}/english-${viewport.name}-${group.name}.png`,
      fullPage: true,
