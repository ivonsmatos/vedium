...
| **WhatsApp** | ⚪ Manual por decisão | — | Botão de link direto (`wa.me/5511911293075`) no site e no e-mail de confirmação de lead | **Decisão de produto (2026-08-07): manter manual** — sem automação por ora, adequado ao volume atual. Se um dia automatizar (lembrete de aula, confirmação), `frappe_whatsapp`/Cloud API da Meta ou provedor BR resolvem; gargalo é Meta/negócio (WABA + número dedicado + templates + opt-in LGPD), não código. Ver [doc 04](04-ecossistema-frappe-oficial.md). |
