# -*- coding: utf-8 -*-
"""Atualiza short_introduction/description (LMS Course) dos 20 cursos com os
textos SEO/GEO preparados em Cliente/Vedium/Artigos/Publicar (arquivo
...
padrão usado em course_translations.py e nos cursos de Espanhol/PLE/Hebraico
criados via oneshot.

A "Meta Description" desse mesmo arquivo-fonte foi aplicada à parte, no
código (vedium_core/course_seo.py + www/curso.py), pois o doctype LMS
Course não tem um campo de meta description dedicado — sem migração de
dados adicional. "Meta Keywords" não tem destino renderizado no site (ver
docstring de course_seo.py) e não é usada por este script.
