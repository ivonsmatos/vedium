...
class TestFullCatalogRegistry(unittest.TestCase):
    ...
    def test_all_courses_have_canonical_prices(self):
    ...
    def test_hebraico_particular(self):
        config = generate_config_for_course("hebraico-particular")
        m1 = config["monthly_prices"][0]
        self.assertEqual(m1["amount"], 560.0) # 140 * 4
        self.assertEqual(m1["classes_per_month"], 4)
        
        a1 = config["annual_prices"][0]
        self.assertEqual(a1["amount"], 466.67)
