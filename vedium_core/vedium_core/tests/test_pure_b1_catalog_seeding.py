...
# Precisamos do script de seeding B1
import vedium_core.scripts.migrations.oneshot.seed_ingls_pr_intermediario_catalog as seed_script
import stripe

class TestB1CatalogSeeding:
    
    @patch('vedium_core.scripts.migrations.oneshot.seed_ingls_pr_intermediario_catalog.frappe')
    @patch('vedium_core.scripts.migrations.oneshot.seed_ingls_pr_intermediario_catalog.stripe')
    ...
    def test_seeding_reuses_stripe_prices_if_exist(self, mock_stripe, mock_frappe):
        ...
        mock_existing_price.product = "prod_UznRRZqfPDfCie"
