...
# Precisamos do script de seeding
import vedium_core.scripts.migrations.oneshot.seed_ingls_elementary_catalog as seed_script
import stripe

class TestA2CatalogSeeding:
    
    @patch('vedium_core.scripts.migrations.oneshot.seed_ingls_elementary_catalog.frappe')
    @patch('vedium_core.scripts.migrations.oneshot.seed_ingls_elementary_catalog.stripe')
    ...
    def test_seeding_reuses_stripe_prices_if_exist(self, mock_stripe, mock_frappe):
        ...
        mock_existing_price.product = "prod_UznRycssZj7MaH"
