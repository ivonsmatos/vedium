...
echo "==========================================="
ssh_exec "sudo mkdir -p /opt/vedium/{deploy/nginx,site,site/css,backups} && sudo mkdir -p /etc/nginx/snippets && sudo chown -R $USER:$USER /opt/vedium" "Criando diretórios"
...
send_file "deploy/Dockerfile" "/opt/vedium/deploy/Dockerfile" "Imagem Frappe/Raven"
send_file "deploy/nginx/${DOMAIN}.conf" "/opt/vedium/deploy/nginx/${DOMAIN}.conf" "Configuração nginx"
send_file "deploy/nginx/security-headers.conf" "/opt/vedium/deploy/nginx/security-headers.conf" "Cabeçalhos de segurança"
send_file "deploy/docker-compose.yml" "/opt/vedium/deploy/docker-compose.yml" "Docker compose"
...
echo "==========================================="
ssh_exec "sudo cp /opt/vedium/deploy/nginx/security-headers.conf /etc/nginx/snippets/vedium-security-headers.conf && sudo cp /opt/vedium/deploy/nginx/${DOMAIN}.conf /etc/nginx/sites-available/${DOMAIN}" "Configurando nginx"
...
echo "==========================================="
ssh_exec "cd /opt/vedium && docker compose -f deploy/docker-compose.yml up -d --build" "Iniciando containers"
ssh_exec "cd /opt/vedium && docker compose -f deploy/docker-compose.yml ps" "Status containers"
